...
# Flask setup for keep-alive (for UptimeRobot)
app = Flask(__name__)
...
# Fun responses for wins and losses
WIN_MESSAGES = [
    "You're on fire! 🔥",
    "Lucky spin! 🍀",
...
async def casino_announcements():
    """Periodically reminds users about the casino features"""
    await bot.wait_until_ready()
    announcements = [
        {"title": "🎰 Feeling Lucky?", 
         "description": f"The Paradox slots are hot! Try your luck with `!bet` and aim for the {JACKPOT_SYMBOL} jackpot!"},
        {"title": "💰 Daily Rewards", 
         "description": "Don't forget to claim your `!daily` reward! Consecutive days build up your streak bonus!"},
        {"title": "🎡 Wheel of Fortune", 
         "description": "Spin the `!wheel` for a chance to multiply your coins!"}
    ]
...
@bot.command()
@commands.cooldown(1, 3, commands.BucketType.user)  # Already has a manual cooldown, but adding decorator for consistency
async def bet(ctx, amount: int, lines: int = 1):
    user = ctx.author.id

    # Cooldown check (3 seconds between bets)
    ...
    # Special messages for win/loss streaks
...
async def daily(ctx):
    ...
    if user in user_daily_claims and now - user_daily_claims[user] < timedelta(days=1):
        ...
        if user in user_daily_claims:
            # Check if last claim was yesterday (with 2-hour grace period)
            yesterday = now - timedelta(days=1)
            day_before = now - timedelta(days=2)

            if day_before <= user_daily_claims[user] <= yesterday + timedelta(hours=2):
                # They claimed yesterday, increase streak
                ...
                if streak_days >= 7:
                    streak_bonus = DAILY_REWARD  # Double reward for 7+ day streak
                    ...
                    streak_bonus = (streak_days * 50)  # 50 coins per day in streak
            else:
                # Streak broken
        ...
        if streak_bonus > 0:
            streak_days = user_streaks[user]["daily"]
            embed.add_field(
                name=f"🔥 {streak_days}-Day Streak Bonus", 
...
async def top(ctx):
    ...
    sorted_winners = sorted(user_winnings.items(), key=lambda x: x[1], reverse=True)[:5]  # Limit to top 5
...
async def wheel(ctx, amount: int):
    """Spin the wheel of fortune to win prizes!"""
    ...
    # Wheel segments (multiplier, probability weight)
    wheel = [
        (0, 20),    # Lose
        (0.5, 15),  # Half back
        (1, 30),    # Money back
...
# Keep-alive task to prevent Replit from stopping the bot
async def keep_alive():
    while True:
        print(f"Bot is alive at {datetime.now()}")
        await asyncio.sleep(300)  # Print every 5 minutes to keep Replit active

# Asynchronous bot start with rate limit handling
async def start_bot():
    try:
        print("Starting bot...")
        ...
        if e.status == 429:  # Rate limit error
            ...
            await start_bot()  # Retry after waiting
        else:
...
if __name__ == "__main__":
    ...
    loop.create_task(save_data())  # Add save task
    try:
        loop.run_forever()
    except KeyboardInterrupt:
        print("Shutting down...")
